...
library(stringr)
temp <- list.files("output_phospho",recursive = T,full.names = T,pattern = "Phospho")
...
for(i in temp) {
    ...
    out.dir = 'tables/phospho_path/'
    if(!dir.exists(out.dir))
        dir.create(out.dir,recursive = T)
    
    t.address   <- paste0(out.dir,t.address)
...
for(i in temp) {
    ...
    t.address   <- unlist(strsplit(i, "/"))
    t.address   <- tail(t.address,1) 
    out.dir = 'tables/phospho_path_clean/'
    if(!dir.exists(out.dir))
        dir.create(out.dir,recursive = T)
    t.address   <- paste0(out.dir,t.address)
