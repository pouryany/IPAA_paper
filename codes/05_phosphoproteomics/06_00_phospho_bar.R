...
Core_paths0 <- read.csv("tables/phospho_path_clean/PhosphoI45F_vs_Control_diffPathways.csv", row.names = NULL)
...
p1 <- ggplot(Core_paths0_up[30:1,], aes( fct_inorder(Pathway.Description), EffectSize, fill = signedP)) +
    ...
    labs(y = "Effect Size (A\u03B2 42-H vs Control)", fill = "-log10(adj. p)") + ylim(c(0,7))+
    theme_bw() +
...
p2 <- ggplot(Core_paths0_down[(nItems-30):nItems,], aes( fct_inorder(Pathway.Description), EffectSize, fill = signedP)) +
    geom_bar(stat = "identity",position = "dodge", width = 0.8) +
    labs(y = "Effect Size (A\u03B2 42-H vs Control)", fill = "-log10(adj. p)") +
    ylim(c(-7,0))+
    theme_bw() +
...
library(patchwork)
cairo_pdf("figures/Phospho_barplot_down_I45vsCtr.pdf",width = 24,height = 8)
p1+p2
dev.off()
