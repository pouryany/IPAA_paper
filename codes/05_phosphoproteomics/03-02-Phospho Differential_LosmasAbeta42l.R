...
contrast  <- makeContrasts(contrasts=c("ConditionLosma-(ConditionAbeta42)"),
...
sel_labs <- (tT[   tT$adj.P.Val < 0.01 , ]$`Gene Symbol`)

sel_labs <- sel_labs[sel_labs!= ""]
keyvals <- ifelse(
        (tT$adj.P.Val > 0.01 | abs(tT$logFC) < 1), 'grey60',
...
ggsave("figures/Volcano_phospho_proteome_Losma_vs_Abeta42.pdf",p1,
       width = 10, height = 8)
...
write.csv(tT,"tables/DE_phospho/phospho_differential_Losma_Abeta42.csv")
