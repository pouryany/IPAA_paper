...
#Excluding ctrl vs ctrl comparisons

exclude <- grep("I47F_vs_G2B2",all.kat.files,value = T)
all.kat.files <- setdiff(all.kat.files,exclude)
...
write.csv(cor_sq, file = paste0("Output/gene_similarity/",
                                "spearman_cor_AD_model_sq_gene", ".csv"))
